...
const submitBranch = async (e) => {
  ...
  formData.append("id", branch.value.id);
  ...
  loading.value = true;
  const res = await store.updateBranch(formData);
  if (res.success) {
...
onMounted(async () => {
  branch_id.value = router.currentRoute._rawValue.params.id;
  loading.value = true;
  const res = await store.editBranch(branch_id.value);
  if (res.success) {
    branch.value = res.branch;
    logo.value = res.branch.logo;
  } else {
    res.message.forEach((val) => {
      toast.error(val);
    });
  ...
  const res1 = await store.getBranchCategory();
  console.log(res1);
  if (res1.success) {
    branch_categories.value = res1.branch_categories;
  } else {
    ...
    <div class="shadow-md m-5 rounded-md">
      ...
      <form action @submit="submitBranch">
        <div class="p-5 bg-white">
          ...
          >
            ...
            <div class="">
              ...
              <input
                ...
                type="file"
              />
              <img :src="FILE_PATH + logo" class="w-10 h-10" :alt="branch.branch" />
            </div>
            <div>
          ...
          <div v-else>
            <Button :type="'submit'" :label="'Update Branch'" />
          </div>
        </div>
      </form>
    </div>
