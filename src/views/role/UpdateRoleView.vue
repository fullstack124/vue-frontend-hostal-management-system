...
const submitRole = async (e) => {
  ...
  loading.value = true;
  const res = await store.updateRole(data);
  if (res.success) {
...
onMounted(async () => {
  loading.value = true;
  role_id.value = router.currentRoute._rawValue.params.id;
  const res = await store.editRole(role_id.value);
  if (res.success) {
    roles.value.name = res.role.name;
  } else {
